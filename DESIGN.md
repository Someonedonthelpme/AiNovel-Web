...
- **All from sheet is the destination, as stage 3o.** No `scaleFoe`, no
  ...
  prerequisites, below.
- **Combat balance work pauses until a player can recruit companions** (user's
  call, later the same day). That is the rest of 3o — the frailty dial, the
  sheet-against-sheet curve, and re-measuring the tables flagged CONTAMINATED —
  and it now waits on step 9. *Why:* every curve so far is a SOLO climber's, and
  the encounter composition is tuned around exactly that ("action economy decides
  solo fights", `combat/encounter.ts:37`); a party changes the numbers all of it
  rests on, so a floor-1 target set now would be set twice. *What keeps running:*
  every existing combat test and pin — nothing is skipped or disabled, and the
  statblock anchor stays guarded. *The cost, accepted:* stage 4 now comes first,
  so a boss's mutation ships under anchor plus delta and can only shift ability
  scores — not hit points, not AC — until 3o lands.
...
3o. **All from sheet** — planned 2026-09-13, not built. **DEFERRED until step 9
   (companions)**, 2026-09-13: prerequisites 1–2 shipped, the rest waits — see the
   2026-09-13 block above. When it resumes, its curve is measured with the party a
   climber can actually field, not a solo body.
...
4. **SHIPPED 2026-09-14 (without mutation) — documented in ARCHITECTURE §9.**
   **Bosses are notable characters with a mutation** — created by floor
...
7.1. **Grudges travel, and send their people** (added by the user, 2026-09-17).
   ...
   **7.1b SHIPPED 2026-09-17** — journeys (`play/journey.ts`): a grudge sets out, walks on the
   clock, and arriving opens the fight.
   **7.1d SHIPPED 2026-09-17** — stations and who goes (`play/station.ts`). Two readings
   settled while building it: COMING THEMSELVES is open to every station (it is
   what every grudge did before 7.1d, and the approved nerve test has a noble
   come), so the station table lists the acts a station adds; and "merchant = a
   creditor" means the lender's end of an `exchange` role, because "owed coin"
   also caught a servant, whom a master owes wages. A person's trade is now read
   from their sheet when it has one, so a survivor fights with the gear of the
   trade it fled from.
...
7.1f. **SHIPPED 2026-09-17** (`play/journey.ts` `fadeGrudges`, `fadeDays`; `Edge.fedAt`;
   `CHASE_THRESHOLD`). **7.1 is complete.**
   **Grudges fade**, over real days (was 7.1e).
   **Decided with the user, 2026-09-17:**
...
8. **SHIPPED 2026-09-18** (`9662f25` engine and `runParley`; `b4b59df` `hearParley`,
   the server, the composer as speech). **Parley** — a Director turn inside a fight,
   its verdict recorded in the action so the fight stays one replayable event.
   **Decided with the user, 2026-09-18:** three answers, `yields | withdraws | refuses`,
   onto stage 6's `yielded | fled` and a no-op; a kind with no company need cannot
   hear; each foe hears once; a word is a turn; a small schema of its own, not the
   full Director turn; the log shows engine words, never the model's.
   **Not built, by decision:** range or sight, a pack at once, the foe resisting the
   roll, a bonus for a foe near its break line.
