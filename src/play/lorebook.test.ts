...
test('the same object always carries the same story', () => {
  ...
  assert.deepEqual(loreFor(item, 42), loreFor(item, 42));
});

test('two worlds tell different stories about the same object', () => {
  const item = thing('weapon_axe_d8', { foundOn: 9 });
  const a = loreFor(item, 42);
  const b = loreFor(item, 43);
...
test('most things are just things', () => {
  ...
  const withStory = many.filter((i) => hasLore(i, 7)).length;
...
test('a deep find carries more of a story than a first-room one', () => {
  const deep = Array.from({ length: 120 }, (_, i) => loreFor(thing(`d${i}`, { foundOn: 24 }), 5))
    .filter((l) => l !== null);
  const shallow = Array.from({ length: 120 }, (_, i) => loreFor(thing(`d${i}`, { foundOn: 0 }), 5))
    .filter((l) => l !== null);
...
test('a history is always about something this world actually has', () => {
  ...
  for (let n = 0; n < 60; n++) {
    const lore = loreFor(thing(`x${n}`, { foundOn: 12 }), 11);
...
test('reading something that speaks to you meets purpose', () => {
  ...
  const lore = loreFor(item, state.world.seed)!;
...
test('a thing with no story says so rather than pretending', () => {
  ...
  for (let n = 0; n < 200 && !plain; n++) {
    ...
    if (!hasLore(candidate, base.world.seed)) plain = candidate;
  }
