...
function pickTarget(candidates: Combatant[]): Combatant | null {
...
export function autoTurn(rng: Rng, state: CombatState): CombatState {
  ...
  const inReach = pickTarget(attackOptions(current, weapon.id).filter((c) => c.side !== actor.side));
  if (inReach) {
  ...
  const quarry = pickTarget(livingEnemies(current, actor));
  if (quarry) {
    ...
    const nowInReach = pickTarget(attackOptions(current, weapon.id).filter((c) => c.side !== actor.side));
    if (nowInReach) {
